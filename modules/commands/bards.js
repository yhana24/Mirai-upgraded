...
module.exports.run = async function({ api, event }) {
  ...
  try {
    const res = await axios.get(`https://official-api-johnsteve-education.maxx-official.repl.co/bard?ask=hi`);
...
async function convertImageToText(imageURL) {
  const response = await axios.get(`https://api.openai.com/v1/chat/completions?input=${encodeURIComponent(imageURL)}`);
